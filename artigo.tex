...
A prática forense esta relacionado fundamentalmente ao controle da evidência

Conceito de isolamento da área e coleta da evidência.

Conceito de reprodutibilidade da coleta

Concento da cadeia de custódia e a garantia da idoneidade da prova https://qperito.com/2013/11/04/queira-o-sr-perito-explicar-o-que-e-cadeia-de-custodia/
...
Até o momento não encontrei principais requisitos, há requisitos da forense mas eles conflitam com as pectos da núvem.
...
\begin{itemize}
 \item Acessar e coletar as informações de memória das máquinas virtuais em nuvem. 
 \item Reproduzir o processo com os mesmos resultados e conhecer a taxa de erro.
 \item Não violar privacidade ou jurisdição das partes não envolvidas na investigação.
...
mecanismo de detecção de intrusão. Em casos de memória volátil, esta forma de coleta de informação não consegue descrever como era a memória antes do ataque. Esta capacidade é
descrita por alguns autores como \cite{Case2014} como necessária para viabilizar a abordagem de coletar o mínimo necessário para realizar a investigação. A única proposta 
encontrada que leva esta necessidade em consideração é \cite{Dezfouli2012} mas propõe que o dado seja armazenado no próprio dispositivo o que pode levar a perda de 
informações importantes caso a máquina virtual seja despejada do pool e seus recursos liberados.
...
nenhum dos autores cobre o cenário onde a máquina virtual é despejada do pool e os recursos são liberados. Atender este cenário é importante pois com as soluções que escalam
automaticamente, as evidências de uma máquina vítima de um ataque que foi despejada de um pool com a diminuição da demanda será para sempre perdida. Analisando cuidadosamente
as propostas de \cite{Poisel2013}, parece que é possível cobrir o cenário mencionado anteriormente mas ele não dá detalhes da implementação suficientes para termos certeza.
...
O trabalho realizado em \cite{George2012} apresenta um arcabouço para coleta de dados de máquinas virtuais. A solução possui duas formas de acionamento, manual ou 
automática, ambas integradas com algum sistema de detecção de intrusão. O autor sugere que o sistema seja utilizado como um serviço a ser descoberto e acionado sob 
demanda. Quando acionado, escuta a rede, determina qual máquina deve ser objeto de investigação, coleta informações de arquivos de log em localizações pré-determinada, 
tráfego de rede e associa as infomações coletadas ao usuário da respectiva máquina. As evidências coletadas são então armazenadas em local fora da nuvem, com o 
objetivo de evitar problemas de jurisdição e compartilhamento multi-inquilino. Permite usar a própria nuvem como local armazenamento caso desejado (e.g., se não houver 
espaço disponível em ambiente externo).

Das características positivas da proposta temos, a implementação como serviço a ser ofertado pelo provedor de nuvem, o que vai ao encontro da tendêcia de forense como 
serviço (FaaS - Forensics as a Service). A preocupação com a quantidade de dados coletados abordando o problema do crescente volume de dados para análise. Por último a 
capacidade de relacionar ações com o usuário do cliente de nuvem que executou as ações suspeitas.

No que tange limitações, de informação volátil, coleta apenas tráfego de rede, mas não coleta memória. Com a estratégia de acionamento após a detecção da intrusão ele não
consegue descrever, com as evidências, como era o sistema antes do ataque pois não guarda informações deste tempo. Apesar de armazenar evidências fora da nuvem, não são 
apresentados detalhes de como ou se garante que as evidências não foram alteradas ou destruídas no transporte até o local de armazenamento, nem discute-se como controlar 
o acesso a essas evidências. Descuidos nos detalhes de transporte, acesso e armazenamento de evidências podem invalidar o uso da evidência em um processo legal.
