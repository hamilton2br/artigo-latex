...
\includegraphics[center,scale=0.30]{solucao-eng-small.pdf}
\centering
\label{fig:Solucao}
\end{figure}
...
\begin{figure}[tb!]
\footnotesize
\caption{Average time for copying the container 
\marcos{Precisa aumentar a fonte das legendas. Por favor faça esse exercício para TODAS as figuras em vez de esperar eu reclamar uma a uma... Se a fonte está menor do que o obtido com {\footnotesize{footnotes}}, então esta errado. Isso vale para qualquer artigo e qualquer formato, então por favor adicione a sua lista de ``conhecimentos adquiridos durante o mestrado'' - Hamilton: então, eu já tinha aumentado antes :( Fiz de novo me guiando pelo footnote e transformei tudo em vetor (pdf)}
...
\includegraphics[center,scale=0.33]{memoria_copia_ieee.pdf}
\centering
...
\begin{figure}[htb!]
\footnotesize
\caption{Evidence transport time \marcos{Não entendi a razão para começar com 0 samples e ter um tempo de ~9 segundos... Enfim, para transportar 0 samples o tempo deveria ser 0, não? Tem algo muito esquisito aqui... - Hamilton: Ajustei o inicio da série}}
\includegraphics[center,scale=0.35]{evidencia_download_ieee.pdf}
\centering
...
\noindent\texttt{\footnotesize \#reading...\marcos{escreva aqui o comando... usar uma figura pra texto não faz lá muito sentido... - Hamilton: mudei a imagem para PDF}}
...
%\marcos{xxx Not fully revised from this point on}
...
\textcolor{blue}{Despite the successful collection by copying the \textbf{/proc/pid/numa\_maps}, due to a feature of the linux kernel, it was not possible to copy the contents of the process memory from the container via \textit{GDB}.
%
According to \cite{cgroupsxptrace}, this happens because the system calls used by tools such as \textit{GDB} and \textit{htop} were created before the implementation of \textit{cgroups} in the linux \textit{kernel} and thus are not aware of the existence of isolation between processes.
%
When the system call \textit{ptrace}, employed by \textit{GDB}, tries to access an area of memory isolated by \textit{cgroups}, the kernel sends a memory access violation signal, the result is shown in Figure \ref{fig:erro-copia-gdb}.
%
The Docker Engine \cite{capabilities} documentation mentions the command \texttt{--cap-add=SYS\_PTRACE --security-opt-seccomp=unconfined} which allows a tool named \textit{ptrace} employed by \textit{GDB} to access the memory of a process inside the container but does not allow the host machine or other container to access it.
%
