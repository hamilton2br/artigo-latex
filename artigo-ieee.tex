...
A implementação da presente proposta foi configurada de modo a realizar coletas de memória em intervalos de 1 minuto e apagar as coletas que tivessem mais de 5 minutos de existência. 
%
Permitiu-se que a solução \fancyname rodasse por 30 minutos durante o qual foram coletadas métricas de uso de espaço em disco utilizado pelos instantâneos de memória salvos e o tempo de pausa no contêiner necessário para a cópia das mesmas.
%
...
%
O gráfico mostra que o aumento do uso do espaço em disco é linear e o crescimento se interrompe quando é atingido o limite de tempo configurado para a janela pois as coletas com tempo de vida maior que o limite configurado são apagadas do disco. 
%
Podemos concluir que a solução mantem a um nível constante a quantidade de dados que é armazenada permitindo planejar o tamanho necessário para o armazenamento das coletas.
%

A figura \ref{fig:memoria_salva} mostra que as coletas continuaram no disco da máquina mesmo após a remoção dos contêiners. Usando o identificador do contêiner e da imagem como nome do arquivo que armazena o instantâneo da memória conseguimos associar a evidência a sua origem de modo a conseguir reproduzir as condições do ataque.
%
...
Uma preocupação da presente proposta é o impacto que a pausa de um contêiner para coleta de dados pode causar na performance da aplicação executando no mesmo. A tabela \ref{tab:results-conteiner} mostra os tempos de cópia de memória durante o experimento e os dados foram plotados no gráfico \ref{fig:memoria_copia}.
%
