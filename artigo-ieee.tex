...
\textcolor{blue}{A second experiment aimed to determine if it is possible, through the analysis of the collected evidence, to identify malware injection in the container memory.
%
To this end a library named \textbf{libexample.so} simulating a malware was injected into one of the containers.
%
After five minutes of \fancyname collecting evidence, a library mentioned berfore was injected into the memory of one of the containers. 
%
After the injection the solution was allowed to continue collecting for another 5 minutes.
%
...
\textcolor{blue}{With the collection of the \textbf{/proc/pid/numa\_maps} directory, two distinct moments were compared in the life of the container, before and after the injection of the library.
%
Figures \ref{fig:antes-injecao} and \ref{fig:apos-injecao} show \textbf{libexample.so} it is possible to see the malware between addresses \textbf{7fbb52a50000} and \textbf{7fbb52c51000} in the snapshot after the injection
%
Therefore, it is possible to identify the injection of a malware via evidence collected by \fancyname from the \textbf{/proc/pid/numa\_maps} directory, allowing, for example, code injection attacks to be identified.}
...
%
The Docker Engine \cite{capabilities} documentation mentions the command \texttt{--cap-add=SYS\_PTRACE --security-opt-seccomp=unconfined} which allows \textit{ptrace} to access the memory of a process inside the container but does not allow the host machine or other container to access
%
...
%
During the course of this project this unique for of identification was only possible through the hash of the container image. This was the only feature that, submitted to the build process from the same recipe resulted in the same hash of the image.
%
So, the implementation for validating the proposed solution can only collect memory information in user space (\ textit {user space}).
%
