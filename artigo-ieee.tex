...
No caso das soluções em nuvem, não é possível remover o hardware para análise pois ele contém informações de vários usuários, alguns dos quais não estão envolvidos na investigação em curso, fazê-lo levaria a violações de privacidade, o que diminui a credibilidade da evidência. 
%
...
A presente proposta tem como objetivo principal coletar memória de recursos de processamento virtual de modo a conseguir: 
...
%
Além disso, deve-se armazenar a memória coletada de modo a garantir sua integridade e confidencialidade, sem violar a jurisdição ou a privacidade de outros usuários no mesmo servidor físico.

\subsection{Descrição}

Nas soluções com infra-estrutura física, o recursoa é persistente. A associação de uma informação da memória, imagem de um disco ou pacotes trafegando na rede à sua origem é duradoura.
%
Com as soluções de infra virtual, em especial as auto-escaláveis, o recurso deixou de ser persistente e tornou-se volátil. 
%
Para resolver o problema da identificação da fonte é necessário encontrar outro elemento persistente para identificar a fonte da evidência coletada. Para isto para isto a presente proposta usa contêineres (LXC). 
%
Embora o contêiner seja uma peça de software e por consequência também é volátil, a imagem compilada e sua execução na forma de contêiner estão atrelados a um hash que os identificam unicamente. Embora máquinas virtuais também possam ser identificadas por hashes, a escolha do contêiner se justifica pela menor granularidade, é possível ter um contêiner com apenas uma parte da pilha da aplicação tornando mais precisa ainda a relação da evidência com sua fonte.
%
...
A seguir é descrito como a presente proposta resolve os problemas de associação da evidência a origem, volume de dados coletados, garantia de custódia e respeito as diferentes jurisdições e privacidade.

A solução proposta por este trabalho para resolver o problema de associação da evidência a sua origem de modo que o processo seja reprodutível usa contêiners para abrigar programas que realizam processamento. A execução desses contêiners é pausada para a coleta de um instantâneo da memória dos processos sob sua execução.
%
Esta coleta é executada em intervalos de tempo conhecidos de modo a ter um histórico da memória do que esta abrigado no contêiner. Em uma distribuição linux (Ubuntu 14.04) isso foi atingido via cópia do diretório \textbf{\\proc} relacionado aos processos sob o \textbf{cgroup} associado ao contêiner e salvo em disco. 
%
...
%
Tais práticas têm levado ao crescente volume de dados que os investigadores têm que analisar. Há uma vertente na comunidade chamada ``sniper forensics'' na qual se coleta e armazena o suficiente para a investigação. 
%
A solução proposta por este trabalho acompanha esta tendência. 
%
Para resolver este problema foi necessário definir qual a quantidade de dados tida como \textit{suficiente} para uma investigação. 
%
Detectar intrusões na memória de processos depende de termos uma descrição da memória antes e depois da intrusão \cite{Case_Memory_Forensics:2014}. Assim, a solução proposta implementa um log rotativo de instantâneos de memória cobrindo uma quantidade de tempo configurável. 
%
Ao integrar a solução com algum sistema de detecção de ameaça, ao detectar um ataque, o log passa de rotativo a completo permitindo que se conheça o sistema antes e depois do ataque e sua evolução como mostrado na Figura 3. \\
...
De modo a não violar a jurisdição de outros países ou a privacidade de outros usuários, a solução proposta armazena a evidência em um local físico fora da nuvem utilizando como transporte uma conexão segura (TLS). 
%
Para cobrir o requisito de garantir que a evidência não foi destruída, alterada ou acessada por qualquer pessoa a solução proposta por este trabalho usará de armazenamento físico fora da nuvem, o transporte será feito pela camada transporte seguro (\textit{Transport Layer System} -- TLS), um hash da evidência será calculado e o acesso ao mesmo será controlado.
%
...
%
A cópia e gravação do arquivo acontece da seguinte forma: a cada minuto a aplicação pausa o contêiner em questão, tira uma cópia do \textbf{numa\_maps}, salva em um arquivo \textbf{.mem} e concatena com o hash de identificação da imagem. 
%
...
%
Técnicas de investigação de malware que se baseam em informações do kernel space como 	por exemplo, a comparação de informações do bloco do ambiente do processo (\textit{Process Environment Block -- PEB}), que ficam no user space, com informações do descritor de endereços de memória virtual (\textit{Virtual Address Descriptor -- VAD}), que fica no Kernel space não são possíveis. 
%
