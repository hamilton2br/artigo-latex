...
\marcos{OK o tópico: ``continuous collection''. PORÉM: na tabela fala que \cite{VanBaar_FAAS:2014} também cobre esse aspecto, então cadê a discussão sobre isso aqui?!?! - Hamilton: Adicionado}
...
%
The result is that digital forensics laboratories are often overloaded, which leads to backlogs that amount for months \cite{Quick_Increase_Volume_Impact:2014}.
%
...
%
Unfortunately, however, neither of them is viable for used in a cloud scenario.
%
...
%
Conversely, the cloud-oriented work from \cite{Sang_Log_approach:2013} constantly collects information from the virtual machine, not distinguishing between what happened before or after the fact of interest. 
%
...
%
Although this approach aids the problem in the short term, in the long run, with the amount of data continually growing, the problem will recur again.
%
\marcos{Esta frase já é colocada na seção \ref{sec:related-privacy}. Repetir em dois pontos o mesmo problema indica falta de foco... Vai aqui ou lá? Onde faz mais sentido? (obs.: o texto na na seção \ref{sec:related-privacy} me parece mais claro, então onde quer que seja colocado, melhor usar o outro texto) - Hamilton: Concordo} 
%
%In addition, \cite{Sang_Log_approach:2013} depends on the active cooperation of the cloud provider, which may be troublesome because cloud providers' priority in case of attacks would be to guarantee the availability of the service, not to collect evidence. \marcos{Já é falado na }
...
One of the most pertinent parts of forensics work is the handling of the evidence, which must be collected, transported and stored so that they are accepted in a legal process and not have their credibility questioned in the course of it.
%
To achieve this goal, one must ensure the evidence's chain of custody.


In a physical infrastructure, the collection of evidence can be done by removing the physical resource, transporting it to a laboratory and there analyze the data. 
%
To limit the exposure of evidence it may be kept in a safe room, to which access is controlled and it is safe from tampering.
%
In a scenario of cloud computing, especially those of self-scalable infrastructure, there are a number of new challenges. 
%
In contrast with traditional infrastructures, the physical resource can not be removed, these resources may contain information from users not related to the investigation in progress, doing so would constitute a violation of privacy. 
%
The integrity of the evidence also ends up being a non-trivial task, as it needs to be collected, transported and stored without its integrity or content being compromised. 
%
Finally, the volatility of the computational resource turns the verification of the evidence's origin a more complex process, because the resource that generates the evidence may cease to exist some time after the collection takes place \cite{Simou_Cloud_Chlng:2014}.
%
Violation of any of these characteristics may put into question the credibility of the evidence. 
%
Although the admissibility of the evidence in a legal process is a decision of the judge the credibility of the same has a important role in his decision.


Among the papers analyzed, only \cite{Sang_Log_approach:2013} and \cite{VanBaar_FAAS:2014} addresses the issue of chain-of-custody although not completely. 
%
Namely, \cite{Sang_Log_approach:2013} employs hashes to check the integrity of the evidence, allowing the detection of changes. 
%
A limitation however, is that it does not make explicit the mechanisms that could be used to prevent unauthorized access (and thus potential change) to the hashes themselves. 
%
On the other hand, \cite{VanBaar_FAAS:2014} show concerns whith who has access to the evidence but does not describe how the evidence is transported or its integrity is secured.
%
The proposals of the other authors focus only on the technical aspect of the collection, without discussing in detail the chain of custody.
%
In general, these works only mention the need of a forensically acceptable collection process.
...
%
In this work, although a VM-based implementation of the proposed architecture would also be possible, we chose to use containers because its identification is hash calculated based on the containers content as opposed to a random serial alpha-numeric identification used on most VM implementations \cite{dockeridentification}.
%
\marcos{Dizer aqui que focamos em conteineres por ser mais fácil rotulá-los de forma única. A frase original era (e precisa ser mudada para melhorar o contexto: In this work, we focus primarily on containers for our implementation of \fancyname, although a VM-based implementation of the proposed architecture would also be possible - Hamilton: Done, o que acha?}
%
