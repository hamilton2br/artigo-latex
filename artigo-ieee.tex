...
\begin{figure*}[htb!]
\caption{Evolução do uso do espaço em disco com o \fancyname. \marcosT{Aumente o tamanho da fonte, para melhorar a legibilidade. No eixo x, não precisa ir de 1 em 1 se isso ajudar a aumentar a fonte. - Hamilton: Feito}}
...
%
Usando o identificador do contêiner e da imagem destacados no box cinza como nome do arquivo que armazena o instantâneo da memória, consegue-se associar a evidência a sua origem (i.e., a imagem e o contêiner), conforme esperado para uma análise forense.
%
...
\begin{figure*}[htb!]
\caption{Lista de instantâneos de memória. \marcosT{Coloque fundo branco e texto em preto (ou outra cor escura): melhora a visibilidade. Também precisa aumentar a fonte, e, potencialmente, fazer highlight de alguma coisa (para o leitor, parece um monte de caracteres aleatórios...). Se precisar de espaço, pode colocar como ``figure*'', para que a imagem ocupe as duas colunas do documento - Hamilton: Feito. Coloquei o box cinza pois outra cor mais forte começaca a chamar mais atenção que o texto.}}
...
Uma potencial preocupação da presente proposta é que a pausa de um contêiner para coleta de dados pode, em princípio, causar perdas no desempenho da aplicação sendo executada. 
%
...
%
Para contêineres executando um motor de páginas web dinâmicas, como é o caso do experimento em questão, essa latência deve ser pouco perceptível por usuários finais.
%



\marcosT{Essas duas frases não é resultado do experimento, mas apenas característica da sua solução, então não faz muito sentido colocar aqui. Melhor mover essas considerações para o local em que você descreve essas características, fazendo merge com o texto que já esteja lá. Se você já fez essa discussão naquele ponto, então não precisa repetir. Se for fazer o merge, lembre-se (repita 2x ao dia, como um mantra): NÃO USE PRIMEIRA PESSOA EM TEXTOS FORMAIS EM PORTUGUÊS.}
Armazenando apenas os instantâneos de memória mostramos que é possível coletar informações da nuvem sem uma cópia total da máquina assim não violando a privacidade dos não envolvidos na investigação.
%
Armazenando em um disco externo mostramos que é possível evitar problemas de jurisdição relacionadas a localização física de um armazenamento virtual caso os instantâneos permanecessem na nuvem.
%
...
\begin{figure*}[htb!]
\caption{Lista de instantâneos de memória. \marcosT{Plote também o tempo médio juntamente como o desvio padrão da média (pode ser uma média móvel caso acredite que faça muita diferença o início e o restante das medidas): esse tipo de métrica é MUITO mais relevante do que essa listagem, embora manter os pontos atuais também seja interessante - Hamilton: Feito}.}
