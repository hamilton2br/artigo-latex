...
\includegraphics[center,scale=0.33]{evolucao_coleta_ieee.pdf}
\centering
...
\includegraphics[center,scale=0.33]{evidencia_download_ieee.pdf}
\centering
...
%
In addition to collecting the \textbf{/proc/pid/numa\_maps} directory contents, a raw copy of the container's process memory was also acquired using the Linux \textit{GDB} utility and the \textit{nsenter} tool, via the commands shown on Figure \ref{fig:comando-copia}.
...
\textcolor{blue}{And figure \ref{fig:conteudo-memoria-copia-gdb} shows the content of the readable part of the memory at addresses \textbf{0x7f85633b9000} where the library \textbf{libexample.so} simulating a library-based malware is allocated.
%
With the appropriate set of tools, cases of \textit{hollow process injection} could be analyzed.}
%
...
\begin{figure}[htb!]
\footnotesize
\caption{Sample of the library's memory inside the container \marcos{Er... o que texto em português está fazendo nesta figura?!?!?!!? - Hamilton: totalmente alterado}}
