...
(1) identificar a fonte da evidência \marcosR{Não ficou claro: fonte é o que? Usuário? Sistea? VM? Contêiner? Outra coisa? Hamilton: Feito}, mesmo se o recurso virtual não existir mais; 
...
%
\marcosR{Eu tinha dito que uma alternativa para a questão de ``máquina virtual vs. contêiner'' era ``trocar para algo como 'recurso virtual', que tem 'sua \textbf{memória} liberada''. Dizer ``\textbf{recurso} virtual que tem seus \textbf{recursos} liberados'' faz zero sentido... Hamilton: OK}
...
%
\marcosT{Não entendi o ``estão atrelados'': é sempre assim (ou seja, você quis dizer ``são normalmente atrelados'') ou isso é parte da sua proposta (ou seja, você quis dizer ``, na presente proposta, são atrelados'') - Hamilton: Feito}
...
%
\marcosT{Não entendi a frase a seguir. Explique melhor o que você quis dizer com ``contêiner com apenas parte da pilha da aplicação sendo monitorada'' (talvez um exemplo ajude?). Hamilton: Feito}
O contêiner também permite identificar com mais precisão a fonte de uma evidência uma vez que é possível dividir as partes de um sistema em contêineres como por exemplo, um contêiner para o motor de páginas dinâmicas (\textit{Apache}), outro com as lógicas de negócio (\textit{golang}) e um terceiro para um banco de dados (\textit{Cassandra}).

\marcosT{O parágrafo estava muito grande, então quebrei aqui. O problema é que falta uma frase para ligar a frase a seguir ao contexto da discussão... Coloque uma frase aqui, deixando claro qual requisito você quer satisfazer com essa ideia de ``interromper temporariamente a execução do contêiner''. Aplique isso para TUDO que for proposta: o leitor tem que saber de antemão pra que você está fazendo alguma coisa, ou vai ficar se perguntando ``Espera, mas pra que fazer isso?!'' - Hamilton: acho que foi}
Uma cópia de memória não é uma atividade atômica [referência], ela concorre com outros processos. Um desses processos pode ser o código malicioso apagando traços de sua existência da memória do contêiner. 
%
De modo a tornar a cópia da memória atômico, a presente proposta interrompe temporariamente a execução do contêiner, realiza a cópia de sua memória, e em seguida retoma sua execução. 
%
...
%
\marcosR{Use futuro \textbf{apenas} para coisas no futuro... Vai ser possível no futuro ou é possível no presente...? - Hamilton: é possível, OK}
...
A maioria das técnicas forenses praticadas atualmente são voltadas à obtenção da informação em sua totalidade, seja via cópia bit a bit, seja por meio da obtenção do \textit{hardware} físico \cite{Simou_Cloud_Chlng:2014} \cite{Bem_Past_Present_Future:2008}. 
%
Embora tais técnicas possam parecer interessantes à primeira vista, elas são hoje responsáveis por um problema: o crescente volume de informações que os investigadores precisam analisar \cite{Quick_Increase_Volume_Impact:2014}.
%
\marcosR{Use passado \textbf{apenas} para coisas no passado... A proposta adotou essas duas estratégias, porém não adota mais (= passado) ou a proposta adotou e continua adotando essas estratégias (=presente)? - Hamilton: continua adotando, OK}
%Para mitigar essa dificuldade, a presente proposta adotada duas estratégias: a primeira foi a definição de um volume de dados que seria \textit{suficiente} para a realização de uma investigação. A segunda foi definir a idade máxima que a evidência pode ter quando o sistema trabalha em condições normais, isto é, não estando sob ataque.
...
\marcosT{CLAREZA: quais ataques? Onde estão esses objetivos (diga a seção!!!)? Perceba que não tem qualquer seção com esse nome: você espera realmente que o leitor procure no seu texto onde eles estão...?}
Visando avaliar sua eficácia em auxiliar as tarefas a análise e identificação das formas de ataque enumeradas nos objetivos, os métodos propostos foram implementados em uma plataforma de testes.
%
